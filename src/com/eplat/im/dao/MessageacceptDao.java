...
/**
 * @类名称：MessageacceptDao
 * @类描述：消息接收
 * @创建人：代码生成器
 * @创建时间：2017年09月22日 22:24:56
 */
public class MessageacceptDao extends AbstractBaseDao implements BaseDao {
	/**
	 * 
	 * insert
	 * 
	...
	public void insert(Object obj) throws Exception {
		...
		try {
			...
			System.out.println("聊天内容："+bean.getContent());
			String statement = "INSERT INTO IM_MESSAGE_ACCEPT "
					+ "(SEQ_ID,MESSAGE_ID,MESSAGE_TYPE,FROM_ID,FROM_NAME,TO_ID,TO_NAME,CONTENT,SEND_DATE,"
					+ "ACCPET_DATE,ACCEPT_STATE,VIEW_DATE,VIEW_STATE,CLIENT_TYPE,BUSINESS_TYPE)"
					+ " VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
			...
			pstmt.setString(14, bean.getClienttype());
			pstmt.setString(15, bean.getBusinesstype());
	...
	public void insertBatch(List<MessagesendBean> list) throws Exception {
		...
		try {
			String statement = "INSERT INTO IM_MESSAGE_ACCEPT "
					...
					+ "ACCEPT_STATE,VIEW_DATE,VIEW_STATE,CLIENT_TYPE,BUSINESS_TYPE)"
					+ " VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
			pstmt = this.getDBConnection().createPreparedStatement(statement);
	...
	public void update(Object obj) throws Exception {
		...
		try {
			MessagesendBean bean = (MessagesendBean) obj;
			String statement = "UPDATE IM_MESSAGE_ACCEPT SET "
					+ "SEQ_ID=?,MESSAGE_ID=?,MESSAGE_TYPE=?,FROM_ID=?,FROM_NAME=?,TO_ID=?,TO_NAME=?,CONTENT=?,"
					+ "SEND_DATE=?,ACCPET_DATE=?,ACCEPT_STATE=?"
					+ ",VIEW_DATE=?,VIEW_STATE=?,CLIENT_TYPE=?,BUSINESS_TYPE=?"
					+ " WHERE SEQ_ID=?";
			pstmt = this.getDBConnection().createPreparedStatement(statement);
			pstmt.setBigDecimal(1, bean.getSeqid());
			pstmt.setString(2, bean.getMessageid());
			...
			pstmt.setBigDecimal(16, bean.getSeqid());
			pstmt.execute();
		} catch (SQLException e) {
			throw new Exception(e);
		} finally {
	...
	public void delete(Object obj) throws Exception {
		...
		try {
			MessagesendBean bean = (MessagesendBean) obj;
			String statement = "DELETE FROM IM_MESSAGE_ACCEPT "
					+ " WHERE SEQ_ID=?";
			pstmt = this.getDBConnection().createPreparedStatement(statement);
			pstmt.setBigDecimal(1, bean.getSeqid());
			pstmt.execute();
	...
	public List<MessagesendBean> query() throws Exception {
		...
		try {
			String statement = "SELECT * FROM IM_MESSAGE_ACCEPT";
			pstmt = this.getDBConnection().createPreparedStatement(statement);
			...
			while (rs.next()) {
				...
				bean.setClienttype(rs.getString("CLIENT_TYPE"));
	...
	public Object queryPK(Object obj) throws Exception {
		...
		try {
			MessagesendBean bean = (MessagesendBean) obj;
			String statement = "SELECT * FROM IM_MESSAGE_ACCEPT "
					+ " WHERE SEQ_ID=? AND CLIENT_TYPE=?";
			pstmt = this.getDBConnection().createPreparedStatement(statement);
			pstmt.setBigDecimal(1, bean.getSeqid());
			pstmt.setString(2, bean.getClienttype());
			rs = pstmt.executeQuery();
			if (rs.next()) {
				bean.setSeqid(rs.getBigDecimal("SEQ_ID"));
				bean.setMessageid(rs.getString("MESSAGE_ID"));
	...
	public List<MessagesendBean> queryByCause(String sql, Object... sqlParams)
		...
		ResultSet rs = null;
		try {
			String statement = "SELECT * FROM IM_MESSAGE_ACCEPT WHERE 1=1 "
					+ sql;
			pstmt = this.getDBConnection().createPreparedStatement(statement);
