 ...
 * @version
 */
public class SqlServerQueryType {
	/**
	 * 查询SQL语句的FROM部分
	 */
	...
	public SqlServerQueryType(String statement) {
	...
	public String getProcessPageSQL() {
		...
		if (this.currentPage > 1) { // 大于第一页的数据分页处理
			...
			buffer.append(") _ROWNUM,");
			buffer.append(selectPart);
			buffer.append(" FROM ");
			buffer.append(fromPart);
			if (!wherePart.trim().equals("")) { // 有查询条件
				buffer.append(" where " + wherePart);
			}
			if (!groupPart.trim().equals("")) { // 分组
				buffer.append( " GROUP BY " + groupPart);
			}
			buffer.append(" ) _TT WHERE _TT._ROWNUM>");
			buffer.append(String.valueOf((this.currentPage - 1) * this.pageNumber));
			buffer.append(" AND _TT._ROWNUM<="+String.valueOf(this.currentPage * this.pageNumber));
			buffer.append(" ORDER BY "+splitPrefix(this.otherPart));
	...
	public static void main(String[] args) {
		try {
			...
			SqlServerQueryType type = new SqlServerQueryType(statement);
