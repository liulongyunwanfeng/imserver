...
public class JsonConfig {
	...
	private static final String[] DEFAULT_EXCLUDES = new String[] { "class",
	...
	public JsonConfig() {
		this.registerJsonValueProcessor(java.sql.Date.class,
				new JsonDateToStringProcessorImpl());
		this.registerJsonValueProcessor(java.sql.Timestamp.class,
				new JsonDateToStringProcessorImpl());
		this.registerJsonValueProcessor(java.math.BigDecimal.class,
				new JsonBigDecimalToStringProcessorImpl());
		this.registerJsonValueProcessor(java.util.Date.class,
				new JsonDateToStringProcessorImpl());
		this.registerJsonValueProcessor(com.eplat.db.rowset.DataRowSet.class,
				new JsonRowsetToStringProcessorImpl());
	 ...
	 * event triggering is enabled.<br>
	 * [Java -&gt; JSON]
	 * 
	...
	public DefaultValueProcessor findDefaultValueProcessor(Class target) {
		if (!defaultValueMap.isEmpty()) {
			Object key = defaultValueProcessorMatcher.getMatch(target,
					defaultValueMap.keySet());
			DefaultValueProcessor processor = (DefaultValueProcessor) defaultValueMap
					.get(key);
			if (processor != null) {
				return processor;
			}
	...
	public PropertyNameProcessor findJavaPropertyNameProcessor(Class beanClass) {
		if (!javaPropertyNameProcessorMap.isEmpty()) {
			...
			return (PropertyNameProcessor) javaPropertyNameProcessorMap
					.get(key);
	...
	public JsonBeanProcessor findJsonBeanProcessor(Class target) {
		if (!beanProcessorMap.isEmpty()) {
			Object key = jsonBeanProcessorMatcher.getMatch(target,
					beanProcessorMap.keySet());
	...
	public PropertyNameProcessor findJsonPropertyNameProcessor(Class beanClass) {
		if (!jsonPropertyNameProcessorMap.isEmpty()) {
			...
			return (PropertyNameProcessor) jsonPropertyNameProcessorMap
					.get(key);
	...
	public JsonValueProcessor findJsonValueProcessor(Class propertyType) {
		if (!typeMap.isEmpty()) {
			Object key = jsonValueProcessorMatcher.getMatch(propertyType,
					typeMap.keySet());
	...
	public JsonValueProcessor findJsonValueProcessor(Class beanClass,
		...
		jsonValueProcessor = (JsonValueProcessor) beanKeyMap
				.get(beanClass, key);
		if (jsonValueProcessor != null) {
			return jsonValueProcessor;
		}

		jsonValueProcessor = (JsonValueProcessor) beanTypeMap.get(beanClass,
				propertyType);
		if (jsonValueProcessor != null) {
			return jsonValueProcessor;
		}
		...
		Object tkey = jsonValueProcessorMatcher.getMatch(propertyType,
				typeMap.keySet());
	...
	/**
	 * Returns a set of default excludes with user-defined excludes.<br>
	 * Takes into account any additional excludes per matching class. [Java
	 * -&gt; JSON]
	 */
	public Collection getMergedExcludes(Class target) {
		...
		if (!exclusionMap.isEmpty()) {
			Object key = propertyExclusionClassMatcher.getMatch(target,
					exclusionMap.keySet());
	 ...
	 * (objects), which clearly violates the JSON spec.<br>
	 * [JSON -&gt; Java]
	 */
	 ...
	 * as a single element JSONArray (media is the array).<br>
	 * [JSON -&gt; Java]
	 */
	...
	/**
	 * Activate/Deactivate handling this jettison convention when converting to
	 * Java.<br>
	 ...
	 * Activate/Deactivate handling this jettison convention when converting to
	 * Java.<br>
	 * * Jettison states the following JSON {'media':{'title':'hello'}} can be
	 * set as a single element JSONArray (media is the array).<br>
	 * [JSON -&gt; Java]
	 */
	...
	public void addIgnoreFieldAnnotation(String annotationClassName) {
		if (annotationClassName != null
				&& !ignoreFieldAnnotations.contains(annotationClassName)) {
	...
	public void addIgnoreFieldAnnotation(Class annotationClass) {
		if (annotationClass != null
				&& !ignoreFieldAnnotations.contains(annotationClass.getName())) {
	...
	/**
	 * Returns a List of all annotations that mark a field to be skipped when
	 * building.<br>
	 * [Java -&gt; JSON]
	 */
	 ...
	 * @param propertyName
	 *            the name of the property to be removed from the exclusion
	 *            list.
	 */
